...
def main():
    ...
    if productionStoPlanModel.optimize():
        ...
        real_CM_avg_pred = productionStoPlanModel.simulate_schedule(num_exp)
        ...
        real_CM_avg_rolling = productionStoPlanModel.simulate_rolling_schedule(num_exp, epsilon=0) 
        real_CM_avg_rolling_na = productionStoPlanModel.simulate_rolling_schedule(num_exp, epsilon=1.1) 
        ...
        real_CM_avg_pred_na = productionStoPlanModel.simulate_schedule(num_exp) 

        # show results in console 
        print(f"\n\nContribution margin predicted by sampling approximation without non-anticipativity : {predictive_CM}")
        print(f"Contribution margin predicted by sampling approximation with non-anticipativity : {predictive_CM}")
        print(f"Average realized contribution margin of sampling approximation without non-anticipativity: {real_CM_avg_pred}")
        print(f"Average realized contribution margin of sampling approximation with non-anticipativity: {real_CM_avg_pred_na}")
        print(f"Average realized contribution margin of rolling sampling approximation without non-anticipativity: {real_CM_avg_rolling}")
        print(f"Average realized contribution margin of rolling sampling approximation with non-anticipativity: {real_CM_avg_rolling_na}")
